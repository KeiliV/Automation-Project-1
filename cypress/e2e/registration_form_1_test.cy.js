...
describe('This is first test suite, Keili Vutt', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        //2023-11-15 17:24:22
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Keili')
        //cy.get('#lastName').type('Vutt')
        cy.get('[data-testid="lastNameTestId"]').type('Vutt')
        cy.get('input[name="password"]').type('Keili1234')
        cy.get('[name="confirm"]').type('Keili1234')
    ...
    */
    ...
    it('User cannot submit data when password AND confirmation password is absent', () => {
        ...
        // Scroll back to confirm password input field
        ...
        //Password field TEST-2
        it.skip('User cannot submit data when password is absent and confirmation password is filled', () => {
            //Testing if the submit button is disabled when password field is cleared AFTER it all mandatory fields were filled.
            //Result: FAILED
            cy.get('#username').type('johnDoe')
            cy.get('[data-testid="phoneNumberTestId"]').type('10203040')
            cy.get('input[name="password"]').type('Password123')
        ...
        //Password field TEST-3
        it.skip('User cannot submit data when confirmation password is absent and password is filled', () => {
            //This test will pass as submit button is disabled when only confirm password field is cleared.
            //For context: bug only happens when password is not filled -> see previous test
            cy.get('#username').type('johnDoe')
            cy.get('[data-testid="phoneNumberTestId"]').type('10203040')
            cy.get('input[name="password"]').type('Password123')
            cy.get('[name="confirm"]').type('Password123')
